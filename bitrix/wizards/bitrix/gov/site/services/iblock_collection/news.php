...
if ($iblockID)
{
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."about/info/news/index.php", array("NEWS_IBLOCK_ID" => $iblockID));
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."about/info/news/rss.php", array("NEWS_IBLOCK_ID" => $iblockID));
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."info/news/index.php", array("NEWS_IBLOCK_ID" => $iblockID));
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."info/news/rss.php", array("NEWS_IBLOCK_ID" => $iblockID));
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."news/index.php", array("NEWS_IBLOCK_ID" => $iblockID));
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."news/rss.php", array("NEWS_IBLOCK_ID" => $iblockID));
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."about-bitrix/journal_of_operations.php", array("NEWS_IBLOCK_ID" => $iblockID));
	CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."_index.php", array("NEWS_IBLOCK_ID" => $iblockID));
	if ($suffix=='progr')
	{
		CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."program/news.php", array("NEWS_IBLOCK_ID" => $iblockID));
		CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."program/rss.php", array("NEWS_IBLOCK_ID" => $iblockID));
	}
	return;
}

$iblockXMLFile=WIZARD_SERVICE_RELATIVE_PATH."/xml/".LANGUAGE_ID.'/news/news_'.$suffix.'.xml'; 
...
WizardServices::SetIBlockFormSettings($iblockID, array('tabs'=>GetMessage('NEWS_TABS_1').$VIDEO_PROPERTY_ID.GetMessage('NEWS_TABS_2').$PHOTOGALLERY_PROPERTY_ID.GetMessage('NEWS_TABS_3')));
...
while($arElement = $dbElement->Fetch()) {
  ...
  $php_format = $DB->DateFormatToPHP($site_format);
	...
	$element->Update($arElement["ID"], Array("ACTIVE_FROM" => date($php_format, strtotime('-' . (++$i + rand(0, $newsRand)) . ' day'))));
}
