...
class CGossiteMail
{
    function Send($event, $lid, $arFields, $filePath)
    {
        global $DB;
        ...
        {
            // get charset
			$strSql = "SELECT C.CHARSET FROM b_lang L, b_culture C WHERE C.ID=L.CULTURE_ID AND L.LID='$lid' ORDER BY L.DEF DESC, L.SORT";
            ...
            {
                $arFiles[] = array("F_PATH" => $_SERVER['DOCUMENT_ROOT'] . $fPath, "F_LINK" => $f = fopen($_SERVER['DOCUMENT_ROOT'] . $fPath, "rb"));
            }
