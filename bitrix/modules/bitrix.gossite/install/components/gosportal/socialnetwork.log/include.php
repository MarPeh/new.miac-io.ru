...
if (!function_exists('__SLGetLogRecord'))
{
	function __SLGetLogRecord($arEvents, $arParams, $arCurrentUserSubscribe, $arMyEntities, &$arTmpEventsOld, &$arTmpEventsNew, &$current_page_date)
	{
		...
		if ($arEvents["EVENT_ID"] == "blog_post" && $arParams["NEW_TEMPLATE"] == "Y")
		{
		...
		if ($arEvents["ENTITY_TYPE"] == SONET_ENTITY_GROUP)
		{
			...
			if (is_set($arEvents["URL"]) && is_array($arSiteWorkgroupsPage) && array_key_exists(SITE_ID, $arSiteWorkgroupsPage))
		...
		$timeFormated = FormatDateFromDB((array_key_exists("LOG_DATE_FORMAT", $arEvents) ? $arEvents["LOG_DATE_FORMAT"] : $arEvents["LOG_DATE"]), (stripos($arParams["DATE_TIME_FORMAT"], 'a') || ($arParams["DATE_TIME_FORMAT"] == 'FULL' && IsAmPmMode()) !== false ? 'H:MI T' : 'HH:MI'));
