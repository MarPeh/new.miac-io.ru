...
else
{
	...
	?><script>
		...
		BX.message({
			sonetLGetPath: '<?=CUtil::JSEscape('/bitrix/components/gosportal/socialnetwork.log/ajax.php')?>',
			sonetLSetPath: '<?=CUtil::JSEscape('/bitrix/components/gosportal/socialnetwork.log/ajax.php')?>',
			sonetLSessid: '<?=bitrix_sessid_get()?>',
			sonetLLangId: '<?=CUtil::JSEscape(LANGUAGE_ID)?>',
			sonetLSiteId: '<?=CUtil::JSEscape(SITE_ID)?>',
			sonetLNoSubscriptions: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_NO_SUBSCRIPTIONS"))?>',
			sonetLInherited: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_INHERITED"))?>',
			sonetLDialogClose: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_CLOSE_BUTTON"))?>',
			sonetLDialogSubmit: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_SUBMIT_BUTTON"))?>',
			sonetLDialogCancel: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_CANCEL_BUTTON"))?>',
			sonetLDialogUT_Y: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_UT_BUTTON_Y"))?>',
			sonetLDialogUT_N: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_UT_BUTTON_N"))?>',
			sonetLTransportTitle: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_TRANSPORT_TITLE"))?>',
			sonetLTransportUnsubscribe: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_TRANSPORT_UNSUBSCRIBE"))?>',
			sonetLVisibleTitle_Y: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_VISIBLE_TITLE_Y"))?>',
			sonetLVisibleTitle_N: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_VISIBLE_TITLE_N"))?>',
			sonetLTransportTitle_M: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_TRANSPORT_M"))?>',
			sonetLTransportTitle_X: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_DIALOG_TRANSPORT_X"))?>',
			...
			sonetLMessageShow: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_T_MESSAGE_SHOW"))?>',
			sonetLMessageHide: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_T_MESSAGE_HIDE"))?>',
			sonetLMoreWait: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_T_MORE_WAIT"))?>',
			sonetLCounterText1: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_COUNTER_TEXT_1"))?>',
			sonetLMenuTransportTitle: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_MENU_TITLE_TRANSPORT"))?>',
			sonetLMenuVisibleTitle: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_MENU_TITLE_VISIBLE"))?>',
			sonetLMenuFavoritesTitleY: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_MENU_TITLE_FAVORITES_Y"))?>',
			sonetLMenuFavoritesTitleN: '<?=CUtil::JSEscape(GetMessageJS("SONET_C30_MENU_TITLE_FAVORITES_N"))?>',
			sonetLDestinationLimit: '<?=intval($arParams["DESTINATION_LIMIT"])?>',
			...
			if ($arParams["USE_FOLLOW"] == "Y"):
				?>
				, sonetLFollowY: '<?=CUtil::JSEscape(GetMessageJS("SONET_LOG_T_FOLLOW_Y"))?>'
				, sonetLFollowN: '<?=CUtil::JSEscape(GetMessageJS("SONET_LOG_T_FOLLOW_N"))?>'
				<?
			endif;
			?>
		});
							...
							?>">
										...
										if (in_array($arEvent["EVENT"]["EVENT_ID"], array("system", "system_groups", "system_friends")))
										{
											...
											foreach($arEvent["EVENT_FORMATTED"]["DESTINATION"] as $arDestination)
											{
												if (strlen($arDestination["URL"]) > 0)
													$strDestination .= '<a href="'.$arDestination["URL"].'" class="feed-add-post-destination feed-add-post-destination-'.$arDestination["STYLE"].'"><span class="feed-add-post-destination-text">'.$arDestination["TITLE"].'</span></a>';
												else
								...
								)
								{
									?><div class="feed-info-block"><?
										...
										?><div class="feed-post-text-block"><?=CSocNetTextParser::closetags(htmlspecialcharsback($arEvent["EVENT_FORMATTED"]["MESSAGE"]))?><?if($arEvent["EVENT_FORMATTED"]["URL"]):?><a href="<?=$arEvent["EVENT_FORMATTED"]["URL"]?>"><?=GetMessage("SONET_C30_MORE_IMPORTANT")?></a><?endif?><i></i></div><?
									?></div><?
								}
									...
									?><div class="feed-post-item feed-post-add-files">
										...
										?><div class="feed-files-cont">
											<span class="feed-com-file-wrap">
												<span class="feed-con-file-icon feed-file-icon-<?=$file_ext?>"></span><?
												if (
													array_key_exists("URL", $arEvent["EVENT"])
													&& strlen($arEvent["EVENT"]["URL"]) > 0
												)
