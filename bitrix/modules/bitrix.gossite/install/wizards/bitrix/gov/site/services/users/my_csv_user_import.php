...
class CSVUserImport
{
	...
	var $userPropertyName = "UF_STRUCTURE";

	var $arSectionCache = Array();
	var $isUserPropertyCreate = false;

	function __construct($csvFilePath, $delimiter)
	{
	...
	function AttachIBlockSectors($iblockID)
	{
		$iblockID = intval($iblockID);
		if (CModule::IncludeModule("iblock") && $iblockID > 0)
		{
			$dbIblock = CIBlock::GetByID($iblockID);
			if ($dbIblock->Fetch())
				$this->SectorBlockId = $iblockID;
		}
	}
	
	function AttachIBlockShedules($iblockID)
	{
		$iblockID = intval($iblockID);
		if (CModule::IncludeModule("iblock") && $iblockID > 0)
		{
			$dbIblock = CIBlock::GetByID($iblockID);
			if ($dbIblock->Fetch())
				$this->SheduleBlockId = $iblockID;
		}
	}
	
	function AttachIBlockStruc($iblockID)
	{
		$iblockID = intval($iblockID);
		if (CModule::IncludeModule("iblock") && $iblockID > 0)
		{
	...
	function __MyGetIBlockSectionID(&$arFields,$perant_sect,$depth)
	{
		$sectionID=$perant_sect;
		if (array_key_exists("IBLOCK_SECTION_NAME_".$depth, $arFields))
		{
			
			$sectionName = trim($arFields["IBLOCK_SECTION_NAME_".$depth]);
			if (strlen($sectionName)>0)
			{
				$dbSection = CIBlockSection::GetList(Array(), Array("NAME" => $sectionName, "SECTION_ID" => $perant_sect, 'IBLOCK_ID'=>$this->attachIBlockID));
				if ($arGroup = $dbSection->Fetch())
				{
					$sectionID = $arGroup["ID"];
				}
				
				if  (($sectionID==0) or ($sectionID==$perant_sect))
				{
					$arSecFields = Array(
						"ACTIVE" => 'Y',
						"IBLOCK_SECTION_ID" => $perant_sect,
						"IBLOCK_ID" => $this->attachIBlockID,
						"NAME" => $sectionName,
					);
					$bs = new CIBlockSection;
					$sectionID = $bs->Add($arSecFields);
				}
				
				$n_depth=$depth+1;
				$csvSectionCode = "IBLOCK_SECTION_NAME_".$n_depth;
				if (array_key_exists($csvSectionCode, $arFields))
				{
					
					$sectionID=$this->__MyGetIBlockSectionID($arFields,$sectionID,$n_depth);
				}
			}
		}
	...
	function __SectorsToArray()
	{
		$ar_sec=array();
		$arFilter = Array('IBLOCK_ID'=>$this->SectorBlockId);
		$dbsec_list = CIBlockSection::GetList(Array($by=>$order), $arFilter, true);
		while($arsec_res = $dbsec_list->GetNext())
		{
			$ar_sec[]=$arsec_res['ID'];
		}
		return $ar_sec;
	...
	function ImportUser()
	{
		...
		$dbResult = CGroup::GetList($by, $order, Array("STRING_ID" => $arFields["GROUP_CODE"], "STRING_ID_EXACT_MATCH" => "Y"));
		if ($arGroup = $dbResult->Fetch())
			$userGroups[] = $arGroup["ID"];
		if ($arFields["GROUP_CODE"]!='PACIENTS') {
			$filter = Array
			(
				"STRING_ID"=>"SCHEDULE_EDIT"
			);

			$rsGroups = CGroup::GetList(($by="c_sort"), ($order="desc"), $filter);
			$res_ar=$rsGroups->Fetch();
			$userGroups[] = $res_ar["ID"];
		}
		...
		if ($userID > 0)
		{
			...
			$my_sec = $this->__SectorsToArray();
			$cnt = count($my_sec)-1;
			$secters_cnt=rand(1,5);
			
			$us_sectors=array();
			for ($i = 1; $i <= $secters_cnt; $i++) 
			{
				$sect=$my_sec[rand(0,$cnt)];
				if (!in_array($sect, $us_sectors)) $us_sectors[]=$sect;
			}
			
			if (array_key_exists("VIS_STRUC", $arFields))
			{
				$arFilter = Array("IBLOCK_ID"=>$this->StrucBlockId, 'CODE'=>$arFields['VIS_STRUC']);
				$s_res = CIBlockSection::GetList(Array(), $arFilter, false);
				if($s_ob = $s_res->GetNext())
				{
					$v_str=$s_ob['ID'];
				}
			}
			$defaultService='';
			if (array_key_exists("DEFAULT_SERVICE", $arFields)&&!empty($arFields['DEFAULT_SERVICE']))
			{
				$arFilter = Array("IBLOCK_ID"=>$this->ServiceBlockId, 'CODE'=>$arFields['DEFAULT_SERVICE']);
				$s_res = CIBlockElement::GetList(Array(), $arFilter, false);
				if($s_ob = $s_res->GetNext())
				{
					$defaultService=$s_ob['ID'];
				}
			}

			$iblockSectionID = $this->__MyGetIBlockSectionID($arFields,0,1);
			
				$arUpdate = Array(
					'UF_STRUCTURE'=>$iblockSectionID,
					'UF_DISTRICT'=>$us_sectors,
					'UF_JOB'=>$v_str,
					'UF_DEFAULT_SERVICE' => $defaultService,
				);
			...
			if (array_key_exists("SHEDULES", $arFields))
			{ 
				$shedule_array = unserialize(str_replace("'",'"',$arFields["SHEDULES"]));
		
				foreach ($shedule_array as $value) 
				{
					$arFilter = Array("IBLOCK_ID"=>$this->SheduleBlockId, 'CODE'=>$value);
					$s_res = CIBlockElement::GetList(Array(), $arFilter, false, NULL);
					if($s_ob = $s_res->GetNext())
					{
						$db_props = CIBlockElement::GetProperty($this->SheduleBlockId, $s_ob['ID'], array("sort" => "asc"), Array("CODE"=>"SPECIALIST"));
						$sotr=array();
						while($ar_props = $db_props->Fetch())
						{
							$sotr[] = IntVal($ar_props["VALUE"]);
						}
						if (!in_array($userID, $sotr)) $sotr[]=$userID;
						
						CIBlockElement::SetPropertyValues($s_ob['ID'], $this->SheduleBlockId,  $sotr, "SPECIALIST");
					}
				}
			}
		}
