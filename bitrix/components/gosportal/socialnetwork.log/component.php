...
require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/components/gosportal/socialnetwork.log/include.php");
...
)
{
	if ($arParams["SHOW_EVENT_ID_FILTER"] == "Y")
	{
		...
		else
		{
			...
			if (array_key_exists('arSocNetFeaturesSettings',$GLOBALS) && is_array($GLOBALS["arSocNetFeaturesSettings"])) {
		...
		if (array_key_exists('arSocNetLogEvents',$GLOBALS) && is_array($GLOBALS["arSocNetLogEvents"])) {
	...
	{
		$arFilter["<=LOG_DATE"] = ConvertTimeStamp(time()+$arResult["TZ_OFFSET"], "FULL");
	...
	if ($arParams["LOG_ID"] <= 0)
	{
		...
		{
			$rsLogCounter = CSocNetLogCounter::GetList(
				array(
					"USER_ID" => $GLOBALS["USER"]->GetID(),
					"SITE_ID" => SITE_ID,
					"CODE" => $arResult["COUNTER_TYPE"]
				),
				array("LAST_DATE", "PAGE_LAST_DATE_1", "PAGE_SIZE")
			);

			if ($arLogCounter = $rsLogCounter->Fetch())
				$arResult["LAST_LOG_TS"] = MakeTimeStamp($arLogCounter["LAST_DATE"]);

			// $arResult["LAST_LOG_TS"] = CSocNetLogCounter::GetLastDateByUserAndCode($GLOBALS["USER"]->GetID(), SITE_ID, $arResult["COUNTER_TYPE"]);	
	...
	if (
		...
		$arResult["LOG_COUNTER"] = CSocNetLogCounter::GetValueByUserID($GLOBALS["USER"]->GetID(), SITE_ID);
