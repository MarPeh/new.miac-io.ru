...
<div class="photo-filter photo-calendar-on-detaillist">
	<form name="photo_form" class="photo-form" action="<?=$APPLICATION->GetCurPageParam("", array("photo_from", "photo_to", "sessid"))?>" method="post">
		...
		<div class="photo-filter-buttons">
			<input type="submit" class="btn" value="<?=GetMessage("P_FILTER_SHOW")?>" />
			<input type="button" class="btn" onclick="this.form.photo_from.value=''; this.form.photo_to.value=''; this.form.submit();" value="<?=GetMessage("P_FILTER_RESET")?>" />
		</div>
	</form>
</div>
<?
